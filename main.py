...
def main():
    with open(file='./src/logging_config.yaml', mode='r') as f:
        logging_yaml = yaml.load(stream=f, Loader=yaml.FullLoader)
    ...
    parser.add_argument('-c', '--config', help='file with server configuration', default='./src/config.ini')
    
    logging.info('parse arguments')
    args = parser.parse_args()
    logging.info(f"getting args: {args}")

    config = Config(args.directory if args.directory != None else '.')
    config.load(args.config)
    
    conn = psycopg2.connect(dbname='file_server', user='postgres', password='postgres', host='192.168.10.1')
    app = create_web_app(conn)
    web.run_app(app)
